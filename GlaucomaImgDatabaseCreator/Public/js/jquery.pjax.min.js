(function(g){function k(D,E,F){var G=this;return this.on("click.pjax",D,function(I){var H=g.extend({},s(E,F));if(!H.container){H.container=g(this).attr("data-pjax")||G}l(I,H)})}function l(I,E,F){F=s(E,F);var H=I.currentTarget;if(H.tagName.toUpperCase()!=="A"){throw"$.fn.pjax or $.pjax.click requires an anchor element"}if(I.which>1||I.metaKey||I.ctrlKey||I.shiftKey||I.altKey){return}if(location.protocol!==H.protocol||location.hostname!==H.hostname){return}if(H.hash&&H.href.replace(H.hash,"")===location.href.replace(location.hash,"")){return}if(H.href===location.href+"#"){return}if(I.isDefaultPrevented()){return}var J={url:H.href,container:g(H).attr("data-pjax"),target:H};var G=g.extend({},J,F);var D=g.Event("pjax:click");g(H).trigger(D,[G]);if(!D.isDefaultPrevented()){z(G);I.preventDefault();g(H).trigger("pjax:clicked",[G])}}function q(G,D,E){E=s(D,E);var F=G.currentTarget;if(F.tagName.toUpperCase()!=="FORM"){throw"$.pjax.submit requires a form element"}var H={type:F.method.toUpperCase(),url:F.action,data:g(F).serializeArray(),container:g(F).attr("data-pjax"),target:F};z(g.extend({},H,E));G.preventDefault()}function z(D){D=g.extend(true,{},g.ajaxSettings,z.defaults,D);if(g.isFunction(D.url)){D.url=D.url()}var I=D.target;var H=p(D.url).hash;var E=D.context=r(D.container);if(!D.data){D.data={}}D.data._pjax=E.selector;function G(M,K,L){if(!L){L={}}L.relatedTarget=I;var N=g.Event(M,L);E.trigger(N,K);return !N.isDefaultPrevented()}var F;D.beforeSend=function(L,K){if(K.type!=="GET"){K.timeout=0}L.setRequestHeader("X-PJAX","true");L.setRequestHeader("X-PJAX-Container",E.selector);if(!G("pjax:beforeSend",[L,K])){return false}if(K.timeout>0){F=setTimeout(function(){if(G("pjax:timeout",[L,D])){L.abort("timeout")}},K.timeout);K.timeout=0}D.requestUrl=p(K.url).href};D.complete=function(K,L){if(F){clearTimeout(F)}G("pjax:complete",[K,L,D]);G("pjax:end",[K,D])};D.error=function(N,O,L){var K=w("",N,D);var M=G("pjax:error",[N,O,L,D]);if(D.type=="GET"&&O!=="abort"&&M){x(K.url)}};D.success=function(N,M,T){var P=z.state;var S=(typeof g.pjax.defaults.version==="function")?g.pjax.defaults.version():g.pjax.defaults.version;var U=T.getResponseHeader("X-PJAX-Version");var L=w(N,T,D);if(S&&U&&S!==U){x(L.url);return}if(!L.contents){x(L.url);return}z.state={id:D.id||m(),url:L.url,title:L.title,container:E.selector,fragment:D.fragment,timeout:D.timeout};if(D.push||D.replace){window.history.replaceState(z.state,L.title,L.url)}try{document.activeElement.blur()}catch(R){}if(L.title){document.title=L.title}G("pjax:beforeReplace",[L.contents,D],{state:z.state,previousState:P});E.html(L.contents);var O=E.find("input[autofocus], textarea[autofocus]").last()[0];if(O&&document.activeElement!==O){O.focus()}a(L.scripts);if(typeof D.scrollTo==="number"){g(window).scrollTop(D.scrollTo)}if(H!==""){var K=p(L.url);K.hash=H;z.state.url=K.href;window.history.replaceState(z.state,L.title,K.href);var Q=g(K.hash);if(Q.length){g(window).scrollTop(Q.offset().top)}}G("pjax:success",[N,M,T,D])};if(!z.state){z.state={id:m(),url:window.location.href,title:document.title,container:E.selector,fragment:D.fragment,timeout:D.timeout};window.history.replaceState(z.state,document.title)}var J=z.xhr;if(J&&J.readyState<4){J.onreadystatechange=g.noop;J.abort()}z.options=D;var J=z.xhr=g.ajax(D);if(J.readyState>0){if(D.push&&!D.replace){j(z.state.id,E.clone().contents());window.history.pushState(null,"",B(D.requestUrl))}G("pjax:start",[J,D]);G("pjax:send",[J,D])}return z.xhr}function v(D,E){var F={url:window.location.href,push:false,replace:true,scrollTo:false};return z(g.extend(F,s(D,E)))}function x(D){window.history.replaceState(null,"","#");window.location.replace(D)}var i=true;var C=window.location.href;var A=window.history.state;if(A&&A.container){z.state=A}if("state" in window.history){i=false}function b(E){var J=z.state;var D=E.state;if(D&&D.container){if(i&&C==D.url){return}if(z.state&&z.state.id===D.id){return}var F=g(D.container);if(F.length){var K,H=e[D.id];if(z.state){K=z.state.id<D.id?"forward":"back";t(K,z.state.id,F.clone().contents())}var I=g.Event("pjax:popstate",{state:D,direction:K});F.trigger(I);var L={id:D.id,url:D.url,container:F,push:false,fragment:D.fragment,timeout:D.timeout,scrollTo:false};if(H){F.trigger("pjax:start",[null,L]);z.state=D;if(D.title){document.title=D.title}var G=g.Event("pjax:beforeReplace",{state:D,previousState:J});F.trigger(G,[H,L]);F.html(H);F.trigger("pjax:end",[null,L])}else{z(L)}F[0].offsetHeight}else{x(location.href)}}i=false}function d(E){var D=g.isFunction(E.url)?E.url():E.url,H=E.type?E.type.toUpperCase():"GET";var F=g("<form>",{method:H==="GET"?"GET":"POST",action:D,style:"display:none"});if(H!=="GET"&&H!=="POST"){F.append(g("<input>",{type:"hidden",name:"_method",value:H.toLowerCase()}))}var G=E.data;if(typeof G==="string"){g.each(G.split("&"),function(I,J){var K=J.split("=");F.append(g("<input>",{type:"hidden",name:K[0],value:K[1]}))})}else{if(typeof G==="object"){for(key in G){F.append(g("<input>",{type:"hidden",name:key,value:G[key]}))
